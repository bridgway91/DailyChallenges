...
const removeDuplicateIds = (obj) => { // took a bit of thinking, but eventually got there... kinda surprised when it worked right away after typing out last half of it
    let keys = Object.keys(obj).map(s=>Number(s)).sort((a,b)=>b-a)
    let letters = []
    for (let key of keys) {
      let temp = []
      for (let l of obj[key]) {
        if (!letters.includes(l)) temp.push(l)
        letters.push(l)
      }
      obj[key] = temp
    }
