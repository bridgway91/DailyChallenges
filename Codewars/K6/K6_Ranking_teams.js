...
function nbaCup(r, toFind) { // was actually kinda annoying, but not too bad
    if(toFind == '') return ''
    if(r.includes('.')) return `Error(float number):${r.split(',').filter(e=>e.includes('.'))[0]}`
    
    const games = r.split(',').filter(e=>e.includes(toFind+' '))
    if(games.length == 0) return `${toFind}:This team didn't play!`
    
    const scores = games.map(e=>{
      const a = +e.match(/ [0-9]+ /gm)[0].trim()
      const b = +e.match(/[0-9]+$/gm)[0]
      return e.slice(0,3)==toFind.slice(0,3) ? [a,b] : [b,a]
    })
    
    const W = scores.filter(e=>e[0]>e[1]).length
    const D = scores.filter(e=>e[0]==e[1]).length
    const L = scores.filter(e=>e[0]<e[1]).length
    
    return `${toFind}:W=${W};D=${D};L=${L};Scored=${scores.reduce((a,c)=>c[0]+a,0)};Conceded=${scores.reduce((a,c)=>c[1]+a,0)};Points=${W*3 + D*1}`
  }