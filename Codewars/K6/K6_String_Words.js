...
function wordsToObject(input) { // another dumb prob with misleading instructions... above seems to indicate wanting an array of objects, but NOPE! just purely string... so dumb
    const seq = input.split(' ')
    let arr = []
    for (let i=0; i<seq.length; i+=2) {
      arr.push(`{name : '${seq[i]}', id : '${seq[i+1]}'}`)
    }
    return '[' + arr.join(', ') + ']'
  }
...
function wordsToObject(input) { // nice to see that i understand regex enough to immediately see that this works... slightly annoyed i didnt think of it, but i was still hung up on trying to make it an array, so w/e
