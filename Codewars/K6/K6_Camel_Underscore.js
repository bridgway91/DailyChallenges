...
const toUnderScore = name => { // honestly i was a little doubtful this would work and handle all the exceptions properly, but it got done! ... wish it could be all in 1 replace, but a simple toss-in caused problems, so i split it up rather than take a bunch of time trying to sort it out
    return name.replace(/(?<=[a-z]|[A-Z])([0-9])/gm,'_$1').replace(/(?<=[a-z]|[0-9]|[A-Z])([A-Z])/gm,'_$1');
  }

// lots of other regex solns, all seem somewhat unique, so just gonna stick with mine