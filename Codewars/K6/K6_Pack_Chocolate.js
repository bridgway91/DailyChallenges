...
function makeChocolates(small, big, goal) { // idk why, but while conceptually this seems super fking simple, my head was just not in the right space to solve this in programming-logic... kept trying to think of ways like while loops and subtracting from the goal, having certain conditions whether to use a big or small choc, etc... finally switched thought process entirely to just a double loop, starting each at 0 small choc and all big choc, and finding what works, returning -1 if nothing is found
    for (let i = 0; i<=small; i++) {
      for (let j = big; j>=0; j--) {
        if(j*5 + i*2 == goal) return i
      }
    }
    return -1
  }
...
function makeChocolates(s,b,n) { // ok, i actually prefer what i submitted, but this soln is basically what my head was trying to do at first and failed to compute
