...
function shoot(x){ // easy enough
    let score = [0,0]
    for (let s of x) {
      let p1 = s[0]['P1'], p2 = s[0]['P2']
      score[0] += p1.replace(/O/g,'').length * (s[1] ? 2 : 1)
      score[1] += p2.replace(/O/g,'').length * (s[1] ? 2 : 1)
    }
    return (score[0] > score[1]) ? 'Pete Wins!'
         : (score[1] > score[0]) ? 'Phil Wins!'
         : 'Draw!'
  }