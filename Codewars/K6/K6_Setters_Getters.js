...
class Archiver { // so i got most of this from a previous soln, HOWEVER, my own attempt was nearly the exact same as this, and i had a bunch of trouble trying to figure out while mine didnt work... best reason i found was that i was using 'temperature' as both a property and the get/set method name, which it didnt like
    constructor() {
      this.archive = [];
    }
    set temperature(x) {
      this.temp = x
      this.archive.push({date:new Date(), val:x})
    }
    get temperature() {
      return this.temp
    }
