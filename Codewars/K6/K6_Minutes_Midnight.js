...
function minutesToMidnight(d){ // seconds part should ideally be in the first line, but copy+paste caused an error and im in a hurry, so it stays
    const remaining = (24*60) - (d.getHours() * 60 + d.getMinutes())
    return remaining + (d.getSeconds()>=30 ? -1 : 0) + ' minute' + (remaining>1 ? 's' : '')
  }