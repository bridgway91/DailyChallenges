...
function arrange(str) { // was a bit of a headache, partly due to me not wanting to do a simple for-loop at the start... other methods kinda got away from me and temp made me forget what i was trying to do
    let arr = str.split(' ')
    for (let i=0; i<arr.length-1; i++) {
      if (i%2) {
        if (arr[i].length < arr[i+1].length)  {[arr[i],arr[i+1]] = [arr[i+1],arr[i]]}
      } else {
        if (arr[i].length > arr[i+1].length)  {[arr[i],arr[i+1]] = [arr[i+1],arr[i]]}
      }
    }
