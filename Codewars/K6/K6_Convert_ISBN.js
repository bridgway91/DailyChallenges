...
function isbnConverter(isbn) { // lot of instructions for something that seems so simple...
    const d = isbn.replace(/-/g,'').substring(0,9)
    const check = d.split('').reduce((t,e,i)=>t + (i%2 ? e*1 : e*3),38)
    
    return '978-' + isbn.substring(0,12) + (check%10 == 0 ? 0 : 10 - check%10)
  }