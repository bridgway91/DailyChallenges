...
Array.prototype.filter = function (callback, thisArg = this) { // NOT CORRECT, TIMES OUT, IDK WHY, GETS INCORRECT FOR 'SHOULD ONLY PROCESS ARRAY UP TO ORIGINAL LENGTH'
    let filArray = []
    for (let i=0; i<thisArg.length; i++) {
      const res = callback(thisArg[i],i,thisArg)
      if (res) filArray.push(thisArg[i])
    }
    return filArray
  }
...
Array.prototype.filter = function (callback, thisArg) { // iterated my soln till it got close enough to another that it passed
    let len = this.length
    let filArray = []
    for (let i=0; i<len; i++) {
      const res = callback.call(thisArg,this[i],i,this)
      if (i in this && res) {
        filArray.push(this[i])
      }
    }
    return filArray
  }