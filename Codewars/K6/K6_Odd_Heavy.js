...
function isOddHeavy(n){ // another mostly easy problem... did struggle slightly dealing with having no odds or no evens, but got it worked out
    let odds = n.filter(e=>e%2!=0).sort((a,b)=>b-a)
    let even = n.filter(e=>e%2==0).sort((a,b)=>b-a)
  
    return odds.length
      ? even.length
        ? odds.every(num=>num>even[0]) // odds + evens present
        : true // only odds present
      : false // no odds present (doesnt care about # evens)
  }