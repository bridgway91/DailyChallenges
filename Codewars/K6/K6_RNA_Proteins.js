...
function protein(rna) { // relatively easy, brief issue with indexOf's return of -1 somehow being a true boolean, but got it working... also could be even slimmer, but i prefer the breakup
    let aa = [...rna.match(/.{3}/g)]
    let proteins = aa.map(e=>dictionary[e])
    return proteins.indexOf('Stop')>=0 ? proteins.filter((_,i)=>i<proteins.indexOf('Stop')).join('') : proteins.join('')
  }
const dictionary = {
    // Phenylalanine
      'UUC':'F', 'UUU':'F',
...
// only including relevant part, but so smart! does the join and then just splits off where the stop would be
return rna.match(/.{3}/g).map((e) => dict[e]).join('').split('Stop')[0];