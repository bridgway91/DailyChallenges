...
function doors(n){ // simple, though curious if there's a quicker math-focused algorithm possible, rather than a double-loop
    const doorsArray = new Array(n+1).fill(0)
    for (let i=1; i<=n; i++) {
      for (let j=i; j<=n; j=j+i) {
        doorsArray[j] = doorsArray[j] ? 0 : 1
      }
    }
    return doorsArray.reduce((a,b)=>a+b)
  }

// alternatively...

function doors(n){ // its just a floor of the square root... -_-
    return ~~Math.sqrt(n)
  }