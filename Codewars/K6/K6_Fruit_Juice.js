...
function mixFruit (arr) {
    const five = 'banana orange apple lemon grapes'
    const seven = 'avocado strawberry mango'
    return Math.round(arr.map(e=>e.toLowerCase()).reduce((total,fruit)=>total + (five.includes(fruit) ? 5 : seven.includes(fruit) ? 7 : 9),0) / arr.length)
  }
