...
// "Buy: b Sell: s" where b and s are 'double' formatted with no decimal, b representing the total price of bought stocks and s the total price of sold stocks. Example: "Buy: 294990 Sell: 0"
...
function balanceStatements(list){ // while being the only solution i didnt need to look at the answers for during this coding session, it was still extremely annoying trying to match up string representations
    if (list.length == 0) return "Buy: 0 Sell: 0"
    let buy=0, sell=0, bad=[]
    const stocks = list.split(',').map(e=>e.trim().split(' '))
  
    for (let stock of stocks) {
      if (stock[1].includes('.') || !stock[2].includes('.') || stock.length != 4) {
        bad.push(stock)
      } else {
        if (stock[3] == 'B') {
          buy += (Number(stock[1]) * Number(stock[2]))
        } else {
          sell += (Number(stock[1]) * Number(stock[2]))
        }
      }
    }
    
    bad = bad.length ? '; ' + `Badly formed ${bad.length}: ` + bad.map(e=>e.join(' ') + ' ;').join('') : ''
  
    return `Buy: ${Math.round(buy)} Sell: ${Math.round(sell)}${bad}`
  }