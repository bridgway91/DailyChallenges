...
function posAverage(s) { // i feel like i had a lot more trouble with this than I should have had...
    let sets = s.split(', ')
    const n = sets.length
    const combs = (n * (n-1)) / 2
    let matches = 0
    ...
    while (sets.length > 1) {
      let first = sets.shift()
      for (let i of sets) {
        for (let d = 0; d < first.length; d++) {
          if (first[d] == i[d]) matches++
        }
      }
    }
    
    return Number.parseFloat(matches / (sets[0].length * combs) * 100).toFixed(10)
  }


// alternatively...

function posAverage(s) { // cleanest other soln
    let arr = s.split(/,\W/), pos = 0, total = 0
    for (let i = 0; i < arr.length; i++) {
      for (let j = i + 1; j < arr.length; j++) {
        for (let k = 0; k < arr[i].length; k++) {
          if (arr[i][k] === arr[j][k]) pos++
          total++ // just a counter for total comparisons made, neat!
        }
      }
    }
