...
function prod2sum(a, b, c, d) { // got all possible solns easily enough, but not with the limitation on line 3, had to look at solns cuz was too tired
    let e = Math.abs(a*d - b*c)
    let f = Math.abs(b*d + a*c)
    let g = Math.abs(a*d + b*c)
    let h = Math.abs(b*d - a*c)
    let p = [e,f,g,h].sort((a,b)=>a-b)
  
    return p[0]==p[1] ? [[p[0],p[3]]] : [[p[0],p[3]],[p[1],p[2]]]
  }