...
function menStillStanding(cards) { // probly couldve been done by just a general counter, or something, but this seemed most straight-forward
    let A = new Array(11).fill(2)
    let B = new Array(11).fill(2)
  
    for (let card of cards) {
      const t = card[0]
      const c = card[card.length-1]
      const n = card.match(/\d+/g)[0]
      ...
      if(A.filter(e=>e>0).length < 7 || B.filter(e=>e>0).length < 7) break
    }
    
    return [A.filter(e=>e>0).length, B.filter(e=>e>0).length]
  }