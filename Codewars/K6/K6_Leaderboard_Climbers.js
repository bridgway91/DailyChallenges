...
function leaderboardSort(leaderboard, changes) { // despite the simple code, actually took me a good bit
    for (let c of changes) {
      const person = c.split(' ')[0]
      const curPos = leaderboard.indexOf(person)
      const newPos = curPos - Number(c.split(' ')[1])
      
      leaderboard.splice(curPos,1)
      leaderboard.splice(newPos,0,person)
    }
