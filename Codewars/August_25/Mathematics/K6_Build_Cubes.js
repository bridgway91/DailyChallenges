...
function findNb(m) { // this one took some work because I had not looked up the sum equation... after getting that was simlple
  let n = 1;
  while (true) {
    const sum = (n * (n + 1) / 2) ** 2;
    if (sum === m) return n;
...
function findNb(m) { // some bad practices, but good method, should be considered...
  var n = 0
  while (m > 0) m -= ++n**3
  return m ? -1 : n
}