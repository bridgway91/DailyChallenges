...
function Bus() { // instructions were a bit unclear to me, so CGPT helped and thn just spit this out... dont have the energy to solve myself, so just used this, and its easy enough to follow
    this.stops = {};  // station name -> array of callbacks
  }
  Bus.prototype.subscribe = function(name, callback) {
    if (!this.stops[name]) this.stops[name] = [];
    this.stops[name].push(callback);
  }
  Bus.prototype.unsubscribe = function(name, callback) {
    if (!callback) {
      delete this.stops[name];  // remove all if no callback provided
    } else {
      this.stops[name] = this.stops[name].filter(cb => cb !== callback);
    }
  }
  Bus.prototype.emit = function(name, ...args) {
    if (this.stops[name]) {
      this.stops[name].forEach(cb => cb(...args));
    }
  }
