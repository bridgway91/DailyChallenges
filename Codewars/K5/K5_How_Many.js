...
function maxSumDig(nmax, maxsm) { // simple enough, just glad didnt have to deal w/ performance problems
    let match = []
    for (let i=1000; i<=nmax; i++) {
      let digits = String(i).split('').map(e=>+e)
      let m = true
      for (let j=0; j<=digits.length-4; j++) {
        let nums = digits.slice(j,j+4)
        let sum = nums.reduce((a,b)=>a+b)
        if(sum>maxsm) {m=false}
      }
      if(m) {match.push(i)}
    }
    
    const a = match.length
    const c = match.reduce((i,j)=>i+j)
    const mean = c/a
    const b = match.reduce((acc,cur)=>Math.abs(cur-mean)<Math.abs(acc-mean) ? cur : acc)
