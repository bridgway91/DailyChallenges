...
function procArrInt(listNum) { // pretty simple
    const n1 = listNum.length
    let divs = listNum.map(e=>[e,getDivs(e).length])
    const n2 = divs.filter(e=>e[1]==2).length
    const n3a = Math.max(...divs.map(e=>e[1]))
    const n3b = divs.filter(e=>e[1]==n3a).map(e=>e[0]).sort((a,b)=>a-b)
