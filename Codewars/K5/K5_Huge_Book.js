...
function pageDigits(n) { // count += (min(n, end_of_block) - start_of_block + 1) × digits_per_page
  n = BigInt(n); // ensure BigInt
  let total = 0n;
  ...
  for (let d = 1n; ; d++) {
    ...
    let end = (10n ** d) - 1n;
    let blockEnd = n < end ? n : end;

    let pagesInBlock = (blockEnd - start + 1n);
    total += pagesInBlock * d;
  }
