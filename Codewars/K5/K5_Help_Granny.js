...
function tour(friends, fTowns, distTable) { // not too hard, just a little annoying with how the inputs were handled
    const o = {}
    for (let f of fTowns) {
      let d = distTable[distTable.indexOf(f[1])+1]
      o[f[0]] = [f[1],d]
    }
  
    let t = friends.filter(e=>o[e]).map((e,i,a)=>{
      if(i==0) return o[e][1]
      let a2 = Math.pow(o[friends[i-1]][1],2)
      let c2 = Math.pow(o[e][1],2)
      return i == a.length-1 ? Math.sqrt(c2-a2) + o[e][1] : Math.sqrt(c2-a2)
    })
  
    return Math.floor(t.reduce((a,b)=>a+b))
  }