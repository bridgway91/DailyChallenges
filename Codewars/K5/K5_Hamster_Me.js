...
const alph = 'abcdefghijklmnopqrstuvwxyz' // pretty simple

function hamsterMe(code, message) {
  let c = [...new Set(code)].sort((a,b)=>a.localeCompare(b))

  c = c.map((e,i)=>{
    if(i==c.length-1) { return alph.slice(alph.indexOf(e)) + alph.slice(0,alph.indexOf(c[0])) }
    else { return alph.slice(alph.indexOf(e),alph.indexOf(c[i+1])) }
  })
  
  return message.split('').map(e=>{
    let group = c.filter(a=>a.includes(e))[0]
    return group[0] + (group.indexOf(e)+1)
  }).join('')
}
