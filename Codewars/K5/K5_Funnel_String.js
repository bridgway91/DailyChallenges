...
function funnelOut(funnel) { // basically my pseudo-code turned into actual code via CGPT, i knew the mindset for how to solve, but didnt want to type it all out, plus it thought of some things i probably wouldve forgotten or taken a bit to figure out
    funnel = [...funnel].reverse(); // reverse to have bottom at index 0
    let conveyor = '';
  
    while (funnel[0][0] != null) {
      conveyor += funnel[0][0];
      funnel[0][0] = null;
      dropdown(funnel);
    }
  ...
  function dropdown(funnel, row = 0, col = 0) {
    if (row + 1 >= funnel.length) return; // no row above
  
    const upper = funnel[row + 1];
    const left = upper[col];
    const right = upper[col + 1];
  
    if (left == null && right == null) return; // no letters to pull down
  
    // choose the smaller letter (if both exist)
    let chosenIndex;
    if (left != null && right != null) {
      chosenIndex = left < right ? col : col + 1;
    } else if (left != null) {
      chosenIndex = col;
    } else {
      chosenIndex = col + 1;
    ...
    funnel[row][col] = upper[chosenIndex];      // move letter down
    upper[chosenIndex] = null;                  // clear original spot
  
    dropdown(funnel, row + 1, chosenIndex);     // recurse upward from where the letter came
  }