// This is the rightful continuation to this easier Kata here and some rules are the same with few substantial alterations. [image]
...
function superStreetFighterSelection(fighters, pos, moves){ // a little long, but not too hard
    let res = [], width = fighters[0].length
    
    for (let m of moves) {
      switch(m) {
        case 'up':
          if(fighters[pos[0]-1] && fighters[pos[0]-1][pos[1]]) {pos[0]--}
          res.push(fighters[pos[0]][pos[1]])
          break
        case 'down':
          if(fighters[pos[0]+1] && fighters[pos[0]+1][pos[1]]) {pos[0]++}
          res.push(fighters[pos[0]][pos[1]])
