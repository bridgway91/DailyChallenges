...
function tongues(str) { // easy
  ...
  const transform = (char) => {
    ...
    if (vowels.includes(lowerChar)) {
      ...
      const newChar = vowels[(index + 3) % vowels.length];
      return isUpper ? newChar.toUpperCase() : newChar;
    }

    if (consonants.includes(lowerChar)) {
      const index = consonants.indexOf(lowerChar);
      const newChar = consonants[(index + 10) % consonants.length];
      return isUpper ? newChar.toUpperCase() : newChar;
    }
