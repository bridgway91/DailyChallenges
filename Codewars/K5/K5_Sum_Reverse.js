...
let special = [], i=45 // so i wasnt sure this would work... just build out all the nums needed at the start and then reference it... tho seems like others did same as me, or even just wrote out the whole list themselves
while(special.length<65) {
  if(i%10 != 0) {
    const r = +String(i).split('').reverse().join('')
    if((i+r)%Math.abs(i-r) == 0) { special.push(i) }
  }
  i++
}
