...
// Careful, your bind implementation should do this too:
...
function canReach(from, to, movements) { // dear god those instructions... despite the scare, it was actually pretty easy, going step-by-step where each function does 1 specific thing, only really got held up on the knightEngine(from, movements) function...
    let possible = knightEngine(from, movements)
  ...
  function moveKnight(from) {
    ...
    return deltas
      .map(([dc, dr]) => [c1 + dc, r1 + dr])
      .filter(([c, r]) => c >= 1 && c <= 8 && r >= 1 && r <= 8);
  }
  
  function moveKnightRandom(from) {
    const moves = moveKnight(from)
  ...
  function bind(fn) {
    return function(arr) {
      return arr.reduce((acc, e) => acc.concat(fn(e)), []);
    };
  }
  ...
  function knightEngine(from, movements) {
    return compose(...Array(movements).fill(bind(moveKnight)), unit)(from);
  }